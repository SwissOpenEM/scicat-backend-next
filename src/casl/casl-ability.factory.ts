...
export class CaslAbilityFactory {
  ...
  isAlwaysFalseQuery(q: Record<string, unknown>): boolean {
    ...
    // This function test for the following expression
    // { $expr: { $eq: [0, 1] } }
    // which is generated by accessibleBy() casl function when no rules match
    // and the user does not have any access at all
    // this is a function that is always false
    //
    // the following test for this expression where the order of o and 1 is not important
    return (
  ...
  jobsMongoQueryReadAccess(user: JWTUser) {
    ...
    // when thr euser does not have permission
    ...
    // which is coded as an empty onbject( {} ),
    // it just returns {}
    ...
    // No access at all
    // returns the expressions provided by accessibleBy() casl function
